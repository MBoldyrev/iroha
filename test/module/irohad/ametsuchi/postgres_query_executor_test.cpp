...
namespace iroha {
  namespace ametsuchi {
    ...
    class GetAccountDetailPagedExecutorTest : public QueryExecutorTest {
     public:
      ...
      void validatePageResponse(
        ...
        // if first record id was specified but not found, retry with first
        // record id unset
        if (expected_data.size() == 0 and expected_total_number > 0
            and (first_record_writer or first_record_key)) {
          return validatePageResponse(response,
                                      std::move(req_writer),
                                      std::move(req_key),
                                      boost::none,
    ...
    TEST_P(GetAccountDetailPagedExecutorTestParametric,
           NonExistentFirstRecord) {
      addDetails(2, 2);
      queryPageAndValidateResponse(
          makeFirstRecordWriterIfNeeded(3), makeFirstRecordKeyIfNeeded(3), 2);
    }
