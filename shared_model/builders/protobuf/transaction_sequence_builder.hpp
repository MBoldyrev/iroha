...
namespace shared_model {
  namespace proto {
    ...
    class [[deprecated]] TransportBuilder<TransactionSequence, SV> {
      TransportBuilder<TransactionSequence, SV>(
     ...
     public:
       ...
       */
      template <class T>
      iroha::expected::Result<TransactionSequence, std::string> build(
          const T &transport) {
        const auto &txs = transport.transactions();
        std::vector<std::shared_ptr<Transaction>> shm_txs;
        ...
        return TransactionSequenceFactory::createTransactionSequence(
            shm_txs,
