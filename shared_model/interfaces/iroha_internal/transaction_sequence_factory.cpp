...
TransactionSequenceFactory::createTransactionSequence(
  ...
  std::unordered_map<types::HashType,
                     std::vector<std::shared_ptr<Transaction>>,
                     types::HashType::Hasher>
      extracted_batches;
  ...
  auto insert_batch =
      [&batches](
          iroha::expected::Value<std::unique_ptr<TransactionBatch>> &&value) {
  ...
  if (transactions.empty()) {
    result.addReason(
        std::make_pair("Transaction collection error",
  ...
  for (const auto &it : extracted_batches) {
    batch_factory->createTransactionBatch(it.second).match(
        insert_batch, [&it, &result](const auto &err) {
          result.addReason(std::make_pair(it.first.toString(),
                                          std::vector<std::string>{err.error}));
        });
  }
