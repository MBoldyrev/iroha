...
namespace shared_model {
  ...
  class ProtoTransportFactory
      : public AbstractTransportFactory<Interface,
                                        typename Proto::TransportType> {
   public:
    using
        typename AbstractTransportFactory<Interface,
                                          typename Proto::TransportType>::Error;
    using ValidatorType =
        std::unique_ptr<shared_model::validation::AbstractValidator<Interface>>;
    ...
    iroha::expected::Result<std::unique_ptr<Interface>, Error> build(
      ...
      std::unique_ptr<Interface> result = std::make_unique<Proto>(std::move(m));
