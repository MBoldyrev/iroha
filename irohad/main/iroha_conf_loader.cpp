 ...
 private:
  ...
  void getVal(const std::string &path,
    ...
    for (auto it = obj.MemberBegin(); it != obj.MemberEnd(); ++it) {
      Val val;
      getVal(fmt::format("{}[{}]", path, it->name), val, it->value);
      dest.emplace_back(it->name, std::move(val));
    }
  }
...
inline void JsonDeserializerImpl::getVal<IrohadConfig::Crypto>(
  ...
  assert_fatal(
      not dest.providers
          or dest.providers->count(config_members::kCryptoProviderDefault) == 0,
      fmt::format(
...
inline void JsonDeserializerImpl::getVal<IrohadConfig::Crypto::ProviderVariant>(
  ...
  if (type == config_members::kCryptoProviderDefault) {
    ...
    getVal(path, dest.get<IrohadConfig::Crypto::HsmUtimaco>(), src);
  } else {
