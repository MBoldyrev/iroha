...
auto OnDemandOrderingInit::createConnectionManager(
  ...
  return std::make_shared<OnDemandConnectionManager>(
          ...
          .map([this](const auto &event) {
            ...
            switch (event.sync_outcome) {
              case iroha::synchronizer::SynchronizationOutcomeType::kCommit:
                ...
                current_round = nextCommitRound(*last_received_round_);
                break;
              case iroha::synchronizer::SynchronizationOutcomeType::kReject:
                log_->debug("Sync event on {}: reject.", *last_received_round_);
                current_round = nextRejectRound(*last_received_round_);
                break;
            ...
            return OnDemandOrderingGate::RoundSwitch{std::move(current_round),
                                                     event.ledger_state};
          }),
      std::move(cache),
