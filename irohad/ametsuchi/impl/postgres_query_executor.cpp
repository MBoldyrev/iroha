  ...
  namespace ametsuchi {
      ...
      return executeQuery<QueryTuple, PermissionTuple>(
          ...
          [&](auto range, auto &) {
            ...
            std::vector<
                ...
                assets;
            boost::for_each(range_without_nulls, [&assets](auto t) {
              apply(t,
    ...
    QueryExecutorResult PostgresQueryExecutorVisitor::operator()(
      ...
      detail AS (
          ...
          page_limits as (
              select start, start + :page_size as end
                  from coalesce((
                      select rn
          ...
          next_record as (
              select
                  nullif(writer, :first_record_writer) as writer,
                  nullif(key, :first_record_key) as key
              from
                  filtered_plain_data,
                  page_limits
          ...
          page as (
              ...
              from (
                  ...
                  from
                      ...
                      page_limits
                  where
                      coalesce(rn >= page_limits.start, true) and
