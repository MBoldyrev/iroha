  ...
  namespace ametsuchi {
    ...
    std::unique_ptr<MutableStorage> StorageImpl::createMutableStorage(
      ...
      if (not mutable_storage_) {
        ...
        mutable_storage_ = std::make_unique<MutableStorageImpl>(
          ledger_state_,
          std::move(command_executor),
          storage_factory.create(),
          log_manager_->getChild("MutableStorageImpl"));
      } else {
        tryRollback(static_cast<MutableStorageImpl &>(*mutable_storage_).getSession());
      }
      return std::move(mutable_storage_);
    }

    void StorageImpl::releaseMutableStorage(
      std::unique_ptr<MutableStorage> st) {
      mutable_storage_ = std::move(st);
    }
    ...
    std::shared_ptr<PeerQuery> StorageImpl::getPeerQuery() const {
      std::shared_ptr<PeerQuery> pq;
      auto wsv = getWsvQuery();
      if (wsv) {
        pq = std::make_shared<PeerQueryWsv>(std::move(wsv));
      }
      return pq;
    }
